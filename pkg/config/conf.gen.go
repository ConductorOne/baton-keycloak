...
type Keycloak struct {
	KeycloakServerUrl string `mapstructure:"keycloak-server-url"`
...
func (c *Keycloak) GetStringSlice(fieldName string) []string {
...
func (c *Keycloak) GetString(fieldName string) string {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return ""
	}
...
func (c *Keycloak) GetBool(fieldName string) bool {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
		return false
	}
...
func (c *Keycloak) GetStringMap(fieldName string) map[string]any {
	v, ok := c.findFieldByTag(fieldName)
	if !ok {
